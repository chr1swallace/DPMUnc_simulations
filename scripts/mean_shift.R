...
ggsave("plots/mean_shift.pdf", width=3, height=3, units="in", dpi=600)
...
g = ggplot(simulation$df, aes(x=z1, y=z2, colour=factor({class}))) +
  ...
  theme(legend.position = "none",
        ...
        axis.text=element_blank(),
        text=element_text(size=15))
ggsave("plots/mean_shift_with_se.pdf", width=3, height=3, units="in", dpi=600)
...
ggplot(NULL) +
  ...
  theme(axis.title=element_blank(),
        ...
        text=element_text(size=15)) +
  facet_grid(. ~ res)
ggsave("plots/mean_shift_psm_graph.pdf", width=6, height=3, units="in", dpi=600)
...
ggplot(NULL) +
  ...
  theme(legend.position = "none",
        ...
        text=element_text(size=15)) +
  facet_grid(. ~ res)
ggsave("plots/mean_shift_latents_clustermeans_both.pdf", width=6, height=3, units="in", dpi=600)
...
ggplot(NULL) +
  ...
  theme(legend.position = "none",
        ...
        axis.text=element_blank(),
        text=element_text(size=15))
ggsave("plots/mean_shift_latents_clustermeans.pdf", width=3, height=3, units="in", dpi=600)
...
ggplot(NULL) +
  ...
  theme(legend.position = "none",
        ...
        axis.text=element_blank(),
        text=element_text(size=15))
ggsave("plots/mean_shift_latents.pdf", width=3, height=3, units="in", dpi=600)
...
ggplot(NULL) +
  ...
  theme(legend.position = "none",
        ...
        axis.text=element_blank(),
        text=element_text(size=15))
ggsave("plots/mean_shift_latents_clustermeans_empirical.pdf", width=3, height=3, units="in", dpi=600)
...
ggplot(NULL) +
  ...
  theme(legend.position = "none",
        ...
        text=element_text(size=15)) +
  facet_grid(. ~ res)
# Keep as png so we can manually add legend with imagemagick
...
ggplot(NULL) +
  ...
  theme(legend.position = "none",
        ...
        axis.text=element_blank(),
        text=element_text(size=15))
ggsave("plots/mean_shift_latents_clustermeans_empirical_mu1.pdf", width=3, height=3, units="in", dpi=600)


# Plots to do manually to get legend
library(ggplot2)

data = data.frame(matrix(rnorm(1000000), ncol=2))
g = ggplot(data) +
  geom_density_2d(aes(x=X1, y=X2), colour="goldenrod2") +
  theme_minimal()

g + theme_classic() + coord_cartesian(xlim=c(-25, 25), ylim=c(-25, 25))

gap = 0.2
points_df = data.frame(z1=1, z2=gap*2, x1=1, x2=gap*1, zhat1=1, zhat2=gap*0, sigmasq2=1)
labels_df = data.frame(x = c(rep(1.05, 3), rep(1.45, 3)),
                       y = c((0:2) * gap,
                             (0:2) * gap),
                       labels = c("Inferred latent point",
                                  "Observed point",
                                  "Latent point",

                                  "Inferred cluster mean density",
                                  "Empirical mean",
                                  "Cluster mean"))
ggplot(points_df) +
  theme_classic() +
  geom_segment(aes(x=z1, y=z2, xend=x1, yend=x2), arrow=arrow(length = unit(0.01, "npc")), colour="grey") +
  geom_segment(aes(x=x1, y=x2, xend=zhat1, yend=zhat2), arrow=arrow(length = unit(0.01, "npc")), colour="grey") +
  geom_point(aes(x=z1, y=z2), size=2, shape=1) +
  geom_point(aes(x=x1, y=x2, size=sigmasq2), alpha=0.5) +
  geom_point(aes(x=zhat1, y=zhat2), colour = "red", shape="square") +
  geom_point(data = data.frame(mu1 = 1.4, mu2 = gap * 2), size=1, shape=2, mapping=aes(x=mu1, y=mu2)) +
  geom_point(data = data.frame(x1 = 1.4, x2 = gap), mapping=aes(x=x1, y=x2), colour="deepskyblue", size=10, shape="x")  +
  geom_text(data = labels_df, aes(x, y, label=labels), hjust="left") +
  coord_cartesian(xlim = c(0.8, 1.8), ylim=c(-1, 2))
